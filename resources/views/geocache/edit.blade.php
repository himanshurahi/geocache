    ...
    <div class="py-1">
        <div class="w-5/6 mx-auto">
            ...
            <div class="bg-white p-5 shadow-md rounded my-6">
				...
				<form action="/geocache/{{$geocache->id}}" method="POST">
                   @csrf
                   @method("PUT")
                    <input class=" w-full text-base py-2 border-b border-gray-300 focus:outline-none focus:border-indigo-500 rounded" type="text" placeholder="Name" name="name" value="{{$geocache->name}}">
                 <textarea class=" w-full text-base h-48 py-2 mt-4 border-b border-gray-300 focus:outline-none focus:border-indigo-500"  placeholder="Description" name="description" >{{$geocache->description}}</textarea>
